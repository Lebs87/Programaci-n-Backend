...
class Container {
    ...
    try {
      ...
      if (documentInDb.length > 0) {
        if (bcrypt.compareSync(password, documentInDb[0].password)) {
          ...
          logger.info(`el ${username} se logueo con una contraseña incorrecta`)
          return { msg: 'Contraseña incorrecta', result: false }
        }
      }
  ...
  async getUser(username) {
    try {
      ...
      loggererr.error(`Error: ${err} al intentar recuperar el usuario id:${username} de la DB`)
    }
  }
  ...
  async addUser(object) {
    try {
      ...
      await newUser.save()
        .then(user => logger.info(`Se ha agregado a la DB elemento con id: ${user._id}`))
        .catch(err => loggererr.error(`Se ha produciodo error ${err} al intentar agregar un usuario a la DB`))
      return true
    } catch (err) {
      loggererr.error(`Se ha produciodo error ${err} al intentar agregar un usuario a la DB`)
    }
  }
